...
export async function sendTutorMessage(
  ...
  // 1. First attempt: Secure Supabase Edge Function Proxy (keeps API key off client)
  try {
    ...
    // Edge function not yet deployed or unreachable; smoothly proceed to direct client fallback
  }
  ...
  // Candidate fallback list to ensure zero downtime if a model variant is deprecated
...
/**
 * Fallback responses when API key is not set or API fails.
 * Provides basic quantum computing knowledge.
 */
...
**Tip:** Configure your API key in the \`.env\` file for dynamic, real-time AI responses.`;
    }
...
${!API_KEY ? '\n> **Tip:** Set your AI API key in the `.env` file for real-time neural responses.' : ''}
